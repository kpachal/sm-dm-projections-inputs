...
xvals = []
yvals = []
with open('Universal_Zprime_HL_LHC.txt') as f :
    found_table = False
    for line in f:
...
xvals_obs = []
yvals_obs = []
with open('discovery_limit_xsec_HL_LHC.txt') as f :
    found_table = False
    for line in f:
        if line.strip().startswith("(GeV)") :
            found_table = True
...
plt.savefig('dijet_hl-lhc_validation.pdf',bbox_inches='tight')

# And finally save a new output file
np.savez('hl-lhc-dijet.npz',xvals_theory=xvals_np,yvals_theory=yvals_np,xvals_obs=xvals_obs_np,yvals_obs=yvals_obs_np)

...
plot_path = "plots/massmass"

# Couplings to test: you'll get a grid of all combinations
test_gq = [0.25,0.1,0.01]
test_gdm = 1.0
test_gl = [0.1,0.01,0.0]
...
for collider in ["hl-lhc","fcc-hh"] :
  ...
  for coupling in monojet_exclusiondepths_axial.keys() :
    ...
    monojet_contours_axial[coupling] = getContours(monojet_mmed, monojet_mdm, monojet_depth)
  ...
  # TODO FIXME using an incorrect one to save time
  ...
  #drawContourPlot([[monojet_mmed, monojet_mdm, debug_monojetv1]], addPoints = False, this_tag = "monojet_V1",plot_path = plot_path+"/validation/")
  ...
  for coupling in monojet_exclusiondepths_vector.keys() :
    ...
    monojet_contours_vector[coupling] = getContours(monojet_mmed, monojet_mdm, monojet_depth)
    dijet_depth = dijet_exclusiondepths_vector[coupling]
    dijet_contours_vector[coupling] = getContours(target_scan_A2.mmed, target_scan_A2.mdm, dijet_depth)
  ...
  if "hl-lhc" in collider :
      ...
      for coupling in dilepton_exclusiondepths_axial.keys() :
        ...
        if (cleanz.size>0) : dilepton_contours_axial[coupling] = getContours(cleanx, cleany, cleanz)
      for coupling in dilepton_exclusiondepths_vector.keys() :
        dilepton_depth = dilepton_exclusiondepths_vector[coupling]
        cleanx, cleany, cleanz = clean_grid(target_scan_A2.mmed, target_scan_A2.mdm,dilepton_depth)
        if (cleanz.size>0) : dilepton_contours_vector[coupling] = getContours(cleanx, cleany, cleanz)
  ...
  for coupling in dijet_exclusiondepths_axial.keys() :
    ...
    drawContourPlot(grid_list_axial, addPoints = False, this_tag = "axial_gq{0}_gdm{1}_gl{2}".format(coupling[0],coupling[1],coupling[2]),plot_path = plot_path)
    drawContourPlot(grid_list_vector, addPoints = False, this_tag = "vector_gq{0}_gdm{1}_gl{2}".format(coupling[0],coupling[1],coupling[2]),plot_path = plot_path)
